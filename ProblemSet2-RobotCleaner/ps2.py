...
class RectangularRoom(object):
    ...
    def getNumTiles(self):
        ...
        returns: an integer
        """
        self.totalTiles = self.width * self.height
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        return (pos.x < self.width and pos.x >= 0 and \
                pos.y < self.height and pos.y >=0 )
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for k in range(0,num_trials):
        ...
        num_cleanTiles = room.getNumCleanedTiles()
        
        for j in range(0,num_robots):
            robot[j] = robot_type(room, speed)

        min_steps = 0
        while num_cleanTiles < min_coverage * total_tiles:
            for j in range(0,num_robots):
                robot[j].updatePositionAndClean()
            num_cleanTiles = room.getNumCleanedTiles()
            min_steps += 1
        
        num_steps[k] = min_steps
    return float( sum( num_steps.values() )/num_trials )
